...
- **Admin Bookings list (`BookingsNew`) sort + accounting export (Sep 2):** the list default-sorts by `created_at` DESC (newest first) on both Active and Archived tabs, with a small toggle (sort field `created_at`/`rental_start`, direction asc/desc) applied client-side over the already-loaded rows. The **Λογιστήριο** control (month picker + Export) produces a single CSV named `accounting-YYYY-MM.csv` with two labeled sections — **PAID & DEPOSITS** (`payment_status IN ('paid','deposit_paid')`) and **PENDING / UNPAID (outstanding)** (everything else) — both filtered to bookings **CREATED in the selected month** (UTC date boundaries) and **excluding `is_archived=true`** test rows. Fixed columns: `booking_number, customer_name, rental_start, rental_end, total_amount, amount_paid, amount_due, payment_type, payment_channel, payment_status, status, created_at`, where `payment_channel` is derived — **Stripe** if `stripe_payment_intent_id` is present, else **Cash/Manual**. Each section ends with a TOTAL row (settled: sum of `total_amount`; outstanding: sum of `amount_due`). Filters match the owner's manual accounting SQL; verified against Aug 2026 (7 settled / 5 outstanding).
