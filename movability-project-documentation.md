...
20. **Operational flags are not financial filters — and a placeholder email is not a test marker.** The first accounting export excluded `is_archived=true` to skip "test bookings," but **archiving is our routine cleanup for completed bookings** (61 archived vs 10 active) — so the filter silently dropped 9 real paid August bookings, reporting €1,722 when the truth was €2,664. Archive/active is an operational signal; never let it gate a revenue report. Second trap in the same fix: the "test email" `pending@moveability.gr` turned out to be the **placeholder for real manual bookings** whose customer email wasn't captured (5 distinct real customers) — excluding it would have re-created the same drop-real-revenue bug. Rule: for money reports, exclude only what is *provably* fake (the owner's own `kalogeropoulosbill6@gmail.com` test account — verify by pulling the rows, don't assume), include everything operational, and **verify the total against a manual figure before shipping** (here: run the exact export logic as SQL → 16 rows / €2,664.10, and only then trust it). When two of the user's own instructions conflict (exclude email X vs. hit total Y that requires X), surface it rather than silently picking one.
...
- **Admin Bookings list (`BookingsNew`) sort + accounting export (Sep 2, corrected same day):** the list default-sorts by `created_at` DESC (newest first) on both Active and Archived tabs, with a small toggle (sort field `created_at`/`rental_start`, direction asc/desc) applied client-side over the already-loaded rows. The **Λογιστήριο** control (month picker + Export) produces a single CSV named `accounting-YYYY-MM.csv` with two labeled sections — **PAID & DEPOSITS** (`payment_status IN ('paid','deposit_paid')`) and **PENDING / UNPAID (outstanding)** (everything else) — both filtered to bookings **CREATED in the selected month** (UTC date boundaries). **ARCHIVED bookings ARE INCLUDED** — archiving is routine operational cleanup for completed bookings (61 archived vs 10 active), NOT a financial signal; the first cut excluded `is_archived=true` and silently dropped 9 real paid August bookings (→ lesson 20). **Test-row exclusion is by known test EMAIL only** — currently just `kalogeropoulosbill6@gmail.com` (the owner's own test account: 15 bookings, all "Vasileios Kalogeropoulos" variants). ⚠️ **`pending@moveability.gr` is NOT a test email** — it is the placeholder for REAL manual bookings whose customer email wasn't captured (5 distinct real customers: Christos Badavas, Jaycee Wood, Lidia, Maria–365 Alternative Travel, Tony Matz), so it stays IN. Fixed columns: `booking_number, customer_name, rental_start, rental_end, total_amount, amount_paid, amount_due, effective_amount_received, payment_type, payment_channel, payment_status, status, created_at`. `payment_channel` = **Stripe** if `stripe_payment_intent_id` present else **Cash/Manual**. **`effective_amount_received`** handles the pre-Aug-16 webhook artifact where fully-paid Stripe rows have `amount_paid=0`: settled `paid` → `total_amount`, `deposit_paid` → `amount_paid` (so Miky Porat's €300 booking counts as its €90 deposit). Each section ends with a TOTAL row (settled: Σ`total_amount` billed + Σ`effective_amount_received`; outstanding: Σ`amount_due`). **Verified against Aug 2026: 16 settled rows / €2,664.10 received (€2,874.10 billed) / 7 outstanding / €1,013 due** — matches the owner's manual figure.
...
- **Sep 2 (admin chronological sort + monthly accounting export):** Two accounting-facing improvements to the admin Bookings list (`src/pages/admin/BookingsNew.tsx`), frontend-only. **(1) Sort:** the list had no explicit order in the UI (the query ordered by `created_at` desc, but nothing was visible/toggleable). Added a client-side sort over the already-loaded rows — default `created_at` DESC (newest first) on both Active and Archived tabs — plus a small toggle (field `created_at`/`rental_start`, direction asc/desc). **(2) Λογιστήριο export:** a month picker (`<input type="month">`, defaults to the current month) + Export button that builds `accounting-YYYY-MM.csv` reusing the existing CSV `esc`/BOM/blob plumbing. One file, two labeled sections — **PAID & DEPOSITS** (`payment_status IN ('paid','deposit_paid')`) and **PENDING / UNPAID (outstanding)** — both filtered to bookings **created in the selected month** (UTC-date boundaries via `created_at.slice(0,10)`, matching a SQL `>= month AND < next-month`). Added `amount_paid`/`amount_due`/`payment_type`/`stripe_payment_intent_id`/`customer_email` to the page's booking `select` + interface. `payment_channel` derived as **Stripe** when `stripe_payment_intent_id` present else **Cash/Manual**. `npm run build` + `tsc --noEmit` green.
  **Corrected same day (→ lesson 20):** the first cut **excluded `is_archived=true`**, which silently dropped 9 real paid August bookings — archiving is our routine cleanup for completed bookings (61 archived vs 10 active), an operational signal, not a financial one. Fix: **archived rows are now INCLUDED** (confirmed the page already loads all rows — `fetchBookings` has no `is_archived` filter — so no separate fetch was needed). Test-row exclusion switched from `is_archived` to a **known-test-EMAIL list**, and critically `pending@moveability.gr` was found to be a **placeholder for REAL manual bookings** (5 distinct real customers, e.g. Lidia's €184 paid booking), NOT a test marker — so the list is just `kalogeropoulosbill6@gmail.com` (the owner's own test account). Also added the **`effective_amount_received`** column the spec called for, handling the pre-Aug-16 webhook artifact where fully-paid Stripe rows carry `amount_paid=0`: settled `paid` → `total_amount`, `deposit_paid` → `amount_paid` (Miky Porat's €300 → €90 deposit). Section TOTALs now show billed (Σ`total_amount`) + received (Σ`effective_amount_received`) for settled, Σ`amount_due` for outstanding. **Re-verified against Aug 2026 by running the exact export logic as SQL: 16 settled / €2,664.10 received (€2,874.10 billed) / 7 outstanding / €1,013 due — matches the owner's manual figure exactly** (vs the buggy first cut's 7/€1,722).
- **Aug 16 (zone/address consistency — advisory, non-blocking):** Extracted address→zone keyword matching into a shared, pure util `src/lib/deliveryZoneDetect.ts` (returns a slug or **null** on no match). Made it **case- AND accent-insensitive across Latin + Greek** (NFD normalize → strip U+0300–U+036F combining marks), and added Greek-script keywords: Γλυφάδα/Κηφισιά/Βουλιαγμένη/Βούλα→`suburbs-riviera`, Ραφήνα→`rafina-port`, Πειραιά(ς)/λιμάνι→`piraeus-port`, αεροδρόμιο→`athens-airport` (Rafina still tested before the generic port rule). Three consumers, all **advisory — the dropdown/selection stays the source of truth, nothing auto-switches or blocks**: (1) `DeliverySection` (checkout) keeps the athens-city fallback for the auto-detected zone, and shows a gentle amber hint under the zone badge when the typed address keyword-matches a DIFFERENT zone than the active one ("Your address looks like it may be in {zone} ({fee}) — tap Change zone if so"); no keyword match → no hint. (2) `BookingSlideOver` and (3) `BookingsNew` admin detail show a small ⚠ "address may not match zone" badge when the address keywords disagree with the **booked** zone — resolved via a new `delivery_zones(slug)` join on the admin queries (`useAdminBookings` + BookingsNew), so mismatches surface at review, not at delivery. Seeded-cart-zone behavior unchanged. `npm run build` + `tsc` green; logic verified against the Greek-suburb-vs-Athens-City case (hint) and matching address+zone (no hint), plus caps/accents and store-pickup (no false positive). Frontend-only, no edge-function or DB change.
