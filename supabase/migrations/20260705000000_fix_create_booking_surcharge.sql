-- Fix: server-side price validation in create_booking() was missing the delivery
-- time/weekend surcharge (added June 22, frontend-only in DeliverySection.tsx
-- getDeliverySurcharge()). Bookings with a surcharge (Sunday, Saturday evening, or
-- weekday evening) were rejected with "Price mismatch: expected X, got X+surcharge".
--
-- This recreates create_booking() to recompute the surcharge with the SAME logic as
-- the frontend, so expected = equipment tier price + zone fee + surcharge.
-- Surcharge rules (delivery only; store pickup = 0):
--   Sunday (any slot) -> 50; Saturday evening -> 50; weekday/Sat evening slot -> 20; daytime -> 0.
...
BEGIN
  ...
  -- 1b. Compute delivery time/weekend surcharge.
  --     Mirrors frontend getDeliverySurcharge() in DeliverySection.tsx.
  --     DELIVERY only (store pickup = no surcharge). EXTRACT(DOW): 0=Sun, 6=Sat.
  ...
  -- Fold the surcharge into the delivery fee we store & validate against.
